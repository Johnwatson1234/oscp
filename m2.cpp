// 中文乱码问题修复版：在需要输出中文的地方使用宽字符版本的API函数
#include <windows.h>
#include <string>
#include <vector>
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_SIZE:
    {
        ...
        if (hTitle) SetWindowPos(hTitle, nullptr, (w-400)/2, panelY-60, 400, 40, SWP_NOZORDER);
        ...
        int labelW=110, labelH=30, editW=120, editH=30, btnW=120, btnH=32, gap=16, rowY=panelY+30;
        ...
        if (g_hOutput) SetWindowPos(g_hOutput, nullptr, panelX+30, panelY+90, panelW-60, panelH-120, SWP_NOZORDER);
    ...
    case WM_COMMAND:
    {
        if (LOWORD(wParam)==ID_BUTTON_START && HIWORD(wParam)==BN_CLICKED)
        {
            ...
            int threadCount=0;
            try { threadCount = std::stoi(std::wstring(buf)); } catch(...) { MessageBoxW(hwnd,L"请输入有效的正整数。",L"错误",MB_ICONERROR); break; }
            ...
            g_threadHandles.clear(); g_threadIndex.clear();
    ...
    case WM_USER_THREAD_MSG:
    {
        ...
        AppendTextToOutput(L"线程 #" + std::to_wstring(index) + L"，系统线程ID = " + std::to_wstring(tid) + L"\r\n");
        break;
    }
    ...
    case WM_PAINT:
    {
        ...
        RECT rcPanel={(w-760)/2,(h-360)/2,(w+760)/2,(h+360)/2};
...
int APIENTRY wWinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPWSTR lpCmdLine,int nCmdShow)
{
    ...
    if(!RegisterClassExW(&wcex)){MessageBoxW(nullptr,L"窗口类注册失败！",L"错误",MB_ICONERROR); return 1;}

    HWND hwnd=CreateWindowExW(0,L"ThreadDemoWindowClass",L"C++ 多线程演示（Win32）",
        WS_OVERLAPPEDWINDOW & ~WS_MAXIMIZEBOX,CW_USEDEFAULT,0,900,540,nullptr,nullptr,hInstance,nullptr);
    if(!hwnd){MessageBoxW(nullptr,L"创建窗口失败！",L"错误",MB_ICONERROR); return 1;}
